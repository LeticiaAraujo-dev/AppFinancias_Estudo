...
class AppFinanciasEstudoSupabaseUser extends BaseAuthUser {
  AppFinanciasEstudoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> appFinanciasEstudoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppFinanciasEstudoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
